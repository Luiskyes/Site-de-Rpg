...
import { normalizeCustomAbilities } from "./custom-abilities";
import { normalizeAbilityName } from "./ability-book";
import {
  STAMINA_UPGRADE_COST,
  normalizeStaminaUpgradeRolls,
} from "./stamina-upgrades";
...
export function calculateCharacterSheet(character, options = {}) {
  ...
  const boughtAbilities = Array.isArray(character?.boughtAbilities)
    ? character.boughtAbilities.filter(
        (ability) =>
          !initialAbilityKey || normalizeAbilityName(ability) !== initialAbilityKey
      )
    : [];
  const customAbilities = normalizeCustomAbilities(character?.customAbilities);
  ...
  return {
    ...
    progress: {
      ...
      customAbilities,
      staminaUpgradeRolls,
      staminaUpgradeCost: STAMINA_UPGRADE_COST,
      staminaUpgradeTotal: staminaUpgradeRolls.reduce(
