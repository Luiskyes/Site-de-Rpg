import { STAMINA_UPGRADE_COST } from "./stamina-upgrades";
...
export function normalizeWeightKg(weightKg) {
...
export function getExistingAbilityCost(specialTrait) {
  return specialTrait === "prodigio" ? 3 : 5;
}

export function getCustomAbilityCost(specialTrait) {
  return specialTrait === "genio" ? 3 : 5;
}

export function calculateProgressSpent({
  ...
  const existingAbilityCost = getExistingAbilityCost(specialTrait);
  const customAbilityCost = getCustomAbilityCost(specialTrait);

  return (
    Number(spentAttributeUpgrades || 0) * 2 +
    Number(spentSkillUpgrades || 0) * 1 +
    (Array.isArray(boughtAbilities) ? boughtAbilities.length : 0) * existingAbilityCost +
    (Array.isArray(customAbilities) ? customAbilities.length : 0) * customAbilityCost +
    (Array.isArray(staminaUpgradeRolls) ? staminaUpgradeRolls.length : 0) *
